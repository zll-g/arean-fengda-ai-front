...
async function loadRecommends() {
  ...
  for (const field of formStore.template.fields) {
    try {
      const res: any = api.voiceForm.getRecommendedValues(templateId.value, field.fieldCode);
...
async function handleRecordStop() {
  ...
  try {
    ...
    const res: any = api.voiceForm.processAudio(fd);
...
async function handleTextSubmit() {
  ...
  try {
    const res: any = api.voiceForm.processText({
      text,
...
async function handleValidate() {
  try {
    const res: any = api.voiceForm.validateForm(templateId.value, formStore.formValues);
...
async function handleReadback() {
  try {
    const res: any = api.voiceForm.readbackForm(templateId.value, formStore.formValues);
...
async function handleConfirm() {
  ...
  if (formStore.formData?.id) {
    const res: any = api.voiceForm.confirmForm(formStore.formData.id);
    formStore.formData = res.data;
...
async function handleSubmit() {
  ...
  if (formStore.formData?.id) {
    const res: any = api.voiceForm.submitForm(formStore.formData.id);
    formStore.formData = res.data;
...
.form-header {
  ...
  margin-bottom: 16px;
  background:
    radial-gradient(circle at 0 0, rgb(255 138 38 / 8%), transparent 32%), rgb(255 255 255 / 96%);
...
.header-info {
  ...
  > :deep(.el-button.is-text) {
    ...
    &:hover {
      ...
      background: #fff0e2;
      border-color: #ffc58f;
      box-shadow: 0 6px 14px rgb(249 115 22 / 10%);
...
.completion-bar {
  ...
  margin-bottom: 16px;
  background:
    radial-gradient(circle at 100% 0%, rgb(255 138 38 / 6%), transparent 32%),
    rgb(255 255 255 / 96%);
  border: 1px solid #f0dfcf;
  border-radius: 16px;
  box-shadow:
...
.form-field {
  ...
  &:hover {
    background: #fff;
    border-color: #ffc58f;
    box-shadow: 0 9px 22px rgb(249 115 22 / 8%);
    transform: translateY(-1px);
  }
  ...
  &.field-filled {
    ...
    &:hover {
      border-color: #86efac;
      box-shadow: 0 9px 22px rgb(16 185 129 / 8%);
    }
  }
  ...
  &.field-warning {
    ...
    &:hover {
      border-color: #fbbf24;
      box-shadow: 0 9px 22px rgb(245 158 11 / 10%);
    }
  }
...
.form-actions {
  ...
  :deep(.el-button--primary) {
    ...
    border-color: #ff8a26;
    box-shadow: 0 9px 20px rgb(249 115 22 / 20%);

    &:hover:not(.is-disabled) {
      background: linear-gradient(135deg, #ff8a26 0%, #e9680a 100%);
      border-color: #f97316;
      box-shadow: 0 12px 24px rgb(249 115 22 / 26%);
    }
  }
  ...
  :deep(.el-button--success) {
    color: #fff;
    background: linear-gradient(135deg, #ffb15f 0%, #e86f0b 100%);
    border-color: #f97316;
    box-shadow: 0 9px 20px rgb(232 111 11 / 18%);

    &:hover:not(.is-disabled) {
      background: linear-gradient(135deg, #ff9a3d 0%, #d95f06 100%);
      border-color: #e9680a;
      box-shadow: 0 12px 24px rgb(232 111 11 / 24%);
    }
  }

  :deep(.el-button.is-disabled) {
    color: #fff;
    background: linear-gradient(135deg, #e7b991, #d6a27a);
    border-color: #ddb08b;
    box-shadow: none;
  }
}
...
.message-item {
  ...
  &.user {
    ...
    .message-content {
      ...
      box-shadow: 0 8px 18px rgb(249 115 22 / 17%);
    }
...
.record-btn {
  ...
  &.recording {
    ...
    border-color: #dc2626;
    box-shadow: 0 13px 24px rgb(239 68 68 / 25%);
