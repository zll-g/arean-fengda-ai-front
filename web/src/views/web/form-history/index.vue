  ...
  <div class="record-list-page">
    ...
    <div class="filter-bar">
      <el-input
        ...
        :placeholder="t('recordList.searchPlaceholder')"
        :prefix-icon="Search"
        clearable
        class="filter-input"
        @input="handleSearch"
      ...
      <div class="filter-row">
        <el-select
          ...
          :placeholder="t('recordList.allTemplates')"
          clearable
          @change="loadData"
        >
          <el-option
        ...
        <el-select
          ...
          :placeholder="t('recordList.allStatus')"
          clearable
          @change="loadData"
        >
          <el-option :label="t('recordList.status.draft')" value="draft" />
          <el-option :label="t('recordList.status.confirmed')" value="confirmed" />
          <el-option :label="t('recordList.status.submitted')" value="submitted" />
        </el-select>
      </div>
    </div>
    ...
    <div v-loading="loading" class="record-list">
      ...
      >
        <el-table-column :label="t('recordList.table.formNo')" prop="formNo" width="180">
          <template #default="{ row }">
            <span class="form-no-link">{{ row.formNo }}</span>
          </template>
        </el-table-column>

        <el-table-column :label="t('recordList.table.template')" prop="templateName" width="160" />

        <el-table-column
          :label="t('recordList.table.title')"
          prop="title"
          min-width="200"
          show-overflow-tooltip
        />

        <el-table-column :label="t('recordList.table.status')" prop="status" width="100">
          <template #default="{ row }">
        ...
        <el-table-column :label="t('recordList.table.createdAt')" width="170">
          <template #default="{ row }">
            {{ formatDate(row.createdAt) }}
          </template>
        </el-table-column>

        <el-table-column :label="t('recordList.table.updatedAt')" width="170">
          <template #default="{ row }">
      ...
      <div v-else class="mobile-card-list">
        ...
        <el-empty
          ...
          :description="t('recordList.empty.noRecord')"
        />
      </div>
...
import { CopyDocument, Document, Edit, Microphone, Search } from '@element-plus/icons-vue';
...
function handleEdit(row: FormDataType) {
  router.push(`/web/form/${row.templateId}?formDataId=${row.id}`);
}
...
async function handleDelete(row: FormDataType) {
  await ElMessageBox.confirm(
    t('recordList.message.deleteConfirm'),
    t('recordList.message.deleteTitle'),
    {
      type: 'warning',
    },
  );
  ...
  ElMessage.success(t('recordList.message.deleteSuccess'));
...
function getStatusLabel(status: string) {
  const statusMap: Record<string, string> = {
    draft: t('recordList.status.draft'),
    confirmed: t('recordList.status.confirmed'),
    submitted: t('recordList.status.submitted'),
  };
